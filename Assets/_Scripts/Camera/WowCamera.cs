...
public class WowCamera : MonoBehaviour
{
	...
	*/
	...
	void OnEnable(){
		Messenger<Transform>.AddListener("network player was instantiated",FollowThis);
		Messenger<bool>.AddListener("freeze character completely",Freeze);
	}
	void OnDisable(){
		Messenger<Transform>.RemoveListener("network player was instantiated",FollowThis);
		Messenger<bool>.RemoveListener("freeze character completely",Freeze);
	}
 	
	void FollowThis(Transform t){
		if (target==null || !target.gameObject.activeSelf) //mmm
			target = t;
	}
