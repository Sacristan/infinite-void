...
private var playerWindow = Rect (10,90,110,50);

private var serverWindow = Rect (130,90,110,50);
...
function OnGUI() {
	if (isInitialized) {
		...
		playerWindow = GUILayout.Window(2, playerWindow, MakePlayerWindow, "Player Info");
		serverWindow = GUILayout.Window(3, serverWindow, MakeServerWindow, "Server Info");
	}
...
function OnNetworkLoadedLevel ()
{
	...
	if (Network.isServer && !isInitialized) {
		...
		// Add self to list
		AddToList(Network.player, playerName, playerCount++, playerColor);
	}
...
function ReceivePlayer (number: int, r: float, g: float, b: float, name: String, player: NetworkPlayer) {
	AddToList(player, name, number, new Color(r,g,b));
	Debug.Log("Received info on player: " + player);
}
...
function InitializeClient (player: NetworkPlayer, number: int, r: float, g: float, b: float) {
	...
	AddToList(player, playerName, number, new Color(r,g,b));
	isInitialized = true;
}
...
function RequestPlayer (name: String, r: float, g: float, b: float, info: NetworkMessageInfo) {
	...
	if (Network.isServer && playerCount == maxPlayers)
	{
		...
		for (var receiver : CastlePlayer in playerList) {
			for (var playerInstance : CastlePlayer in playerList) {
				if (receiver.player != playerInstance.player && receiver.player.ToString() != "0") {
					Debug.Log("Sending info on player " + playerInstance.player + " to player " + receiver.player);
					networkView.RPC("ReceivePlayer", receiver.player, playerInstance.number, playerInstance.color.r, playerInstance.color.g, playerInstance.color.b, playerInstance.name, playerInstance.player);
				}
			}
		}
...
function AddToList(player: NetworkPlayer, name: String, number: int, color: Color)
{
